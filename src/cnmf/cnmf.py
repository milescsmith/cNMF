...
class cNMF:  # noqa: N801
    def __init__(self, output_dir=None, name=None):
        ...
        if name is None:
            ...
            name = f"{now.strftime('%Y_%m_%d')}_{rand_hash}"
        self.name = name
        self.paths = None
    ...
    def _initialize_dirs(self):
        if self.paths is None:
            ...
            check_dir_exists(self.output_dir.joinpath(self.name))
            check_dir_exists(self.output_dir.joinpath(self.name, "cnmf_tmp"))

            self.paths = {
                "normalized_counts": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    f"{self.name!s}.norm_counts.h5ad",
                ),
                "nmf_replicate_parameters": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    f"{self.name!s}.nmf_params.df.npz",
                ),
                "nmf_run_parameters": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    f"{self.name!s}.nmf_idvrun_params.yaml",
                ),
                "nmf_genes_list": self.output_dir.joinpath(self.name, f"{self.name!s}.overdispersed_genes.txt"),
                "tpm": self.output_dir.joinpath(self.name, "cnmf_tmp", f"{self.name!s}.tpm.h5ad"),
                "tpm_stats": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    f"{self.name!s}.tpm_stats.df.npz",
                ),
                "iter_spectra": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    f"{self.name!s}.spectra.k_%d.iter_%d.df.npz",
                ),
                "iter_usages": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    f"{self.name!s}.usages.k_%d.iter_%d.df.npz",
                ),
                "merged_spectra": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    self.name + f"{self.name!s}.spectra.k_%d.merged.df.npz",
                ),
                "local_density_cache": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    self.name + f"{self.name!s}.local_density_cache.k_%d.merged.df.npz",
                ),
                "consensus_spectra": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    self.name + f"{self.name!s}.spectra.k_%d.dt_%s.consensus.df.npz",
                ),
                "consensus_spectra__txt": self.output_dir.joinpath(
                    self.name,
                    self.name + f"{self.name!s}.spectra.k_%d.dt_%s.consensus.txt",
                ),
                "consensus_usages": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    self.name + f"{self.name!s}.usages.k_%d.dt_%s.consensus.df.npz",
                ),
                "consensus_usages__txt": self.output_dir.joinpath(
                    self.name,
                    f"{self.name!s}.usages.k_%d.dt_%s.consensus.txt",
                ),
                "consensus_stats": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    f"{self.name!s}.stats.k_%d.dt_%s.df.npz",
                ),
                "clustering_plot": self.output_dir.joinpath(self.name, f"{self.name!s}.clustering.k_%d.dt_%s.png"),
                "gene_spectra_score": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    f"{self.name!s}.gene_spectra_score.k_%d.dt_%s.df.npz",
                ),
                "gene_spectra_score__txt": self.output_dir.joinpath(
                    self.name,
                    f"{self.name!s}.gene_spectra_score.k_%d.dt_%s.txt",
                ),
                "gene_spectra_tpm": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    f"{self.name!s}.gene_spectra_tpm.k_%d.dt_%s.df.npz",
                ),
                "gene_spectra_tpm__txt": self.output_dir.joinpath(
                    self.name,
                    f"{self.name!s}.gene_spectra_tpm.k_%d.dt_%s.txt",
                ),
                "starcat_spectra": self.output_dir.joinpath(
                    self.name,
                    "cnmf_tmp",
                    f"{self.name!s}.starcat_spectra.k_%d.dt_%s.df.npz",
                ),
                "starcat_spectra__txt": self.output_dir.joinpath(
                    self.name,
                    f"{self.name!s}.starcat_spectra.k_%d.dt_%s.txt",
                ),
                "k_selection_plot": self.output_dir.joinpath(self.name, f"{self.name!s}.k_selection.png"),
                "k_selection_stats": self.output_dir.joinpath(self.name, f"{self.name!s}.k_selection_stats.df.npz"),
            }
    ...
    def get_nmf_iter_params(
        ...
        if n_completed > 0:
            message = f"""{n_completed} runs already appear completed. If this is unexpected, consider
    ...
    def update_nmf_iter_params(self):
        ...
        """
        _nmf_kwargs = yaml.safe_load(
            open(self.paths["nmf_run_parameters"]),
    ...
    def factorize(
        ...
        if not skip_completed_runs:
            ...
            jobs_for_this_worker = worker_filter(
                run_params.index[run_params["completed"] is False],
                worker_i,
                total_workers,
            )
        ...
        for idx in jobs_for_this_worker:
            ...
            print(f"[Worker {worker_i}]. Starting task {idx}.")
    ...
    def combine_nmf(self, k, skip_missing_files=False):
        run_params = load_df_from_npz(self.paths["nmf_replicate_parameters"])
        print(f"Combining factorizations for {k=}.")

        run_params_subset = run_params[run_params.n_components == k].sort_values("iter")
        combined_spectra = []

        for _, p in run_params_subset.iterrows():
            current_file = Path(str(self.paths["iter_spectra"]) % (p["n_components"], p["iter"]))
            if not current_file.exists():
                if not skip_missing_files:
                    print(f"Missing file: {current_file!s}, run with skip_missing=True to override")
                    raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), current_file)
                else:
                    print(f"Missing file: {current_file!s}. Skipping.")
            else:
                spectra = load_df_from_npz(current_file)
                spectra.index = [f"iter{(p['iter'], t + 1)}_topic{(p['iter'], t + 1)}" for t in range(k)]
                combined_spectra.append(spectra)

        if len(combined_spectra) > 0:
            combined_spectra = pd.concat(combined_spectra, axis=0)
            save_df_to_npz(combined_spectra, str(self.paths["merged_spectra"]) % k)
        else:
            print(f"No spectra found for {k=}")
    ...
    def refit_usage(self, X, spectra):
        ...
        refit_nmf_kwargs = yaml.safe_load(
            open(self.paths["nmf_run_parameters"]),
        )  # Loader=yaml.FullLoader)
        if type(spectra) is pd.DataFrame:
            refit_nmf_kwargs.update({"n_components": spectra.shape[0], "H": spectra.values, "update_H": False})
        else:
            refit_nmf_kwargs.update({"n_components": spectra.shape[0], "H": spectra, "update_H": False})
    ...
    def consensus(
        ...
        merged_spectra = load_df_from_npz(str(self.paths["merged_spectra"]) % k)
        ...
        if not skip_density_and_return_after_stats:
            ...
            topics_dist = None
            if os.path.isfile(str(self.paths["local_density_cache"]) % k):
                local_density = load_df_from_npz(str(self.paths["local_density_cache"]) % k)
            else:
                ...
                save_df_to_npz(local_density, str(self.paths["local_density_cache"]) % k)
            ...
            if l2_spectra.shape[0] == 0:
                msg = "Zero components remain after density filtering. Consider increasing density threshold"
        ...
        save_df_to_npz(rf_usages, str(self.paths["consensus_usages"]) % (k, density_threshold_repl))
        # save_df_to_npz(consensus_stats, self.paths['consensus_stats']%(k, density_threshold_repl))
        save_df_to_text(
            median_spectra,
            str(self.paths["consensus_spectra__txt"]) % (k, density_threshold_repl),
        )
        save_df_to_text(rf_usages, str(self.paths["consensus_usages__txt"]) % (k, density_threshold_repl))
        save_df_to_npz(spectra_tpm, str(self.paths["gene_spectra_tpm"]) % (k, density_threshold_repl))
        save_df_to_text(
            spectra_tpm,
            str(self.paths["gene_spectra_tpm__txt"]) % (k, density_threshold_repl),
        )
        save_df_to_npz(usage_coef, str(self.paths["gene_spectra_score"]) % (k, density_threshold_repl))
        save_df_to_text(
            usage_coef,
            str(self.paths["gene_spectra_score__txt"]) % (k, density_threshold_repl),
        )
        if show_clustering:
            ...
            spectra_order = np.hstack([
                determine_spectra_order(cl, kmeans_cluster_labels.values)
                for cl
                in sorted(set(kmeans_cluster_labels))
            ])
            ...
            dist_im = dist_ax.imshow(D, interpolation="none", cmap="viridis", aspect="auto", rasterized=True)
            ...
            hist_gs = gridspec.GridSpecFromSubplotSpec(3, 1, subplot_spec=gs[1, 3], wspace=0, hspace=0)
            ...
            hist_ax.set_xlabel(
                f"Mean distance to k nearest neighbors\n\n{sum(~density_filter)}/{len(density_filter)} ({100 * (~density_filter).mean()}) spectra above threshold\nwere removed prior to clustering"
            )

            ## Add colorbar
            cbar_gs = gridspec.GridSpecFromSubplotSpec(8, 1, subplot_spec=hist_gs[1, 0], wspace=0, hspace=0)
    ...
    def build_reference(self, k, density_threshold=0.5, target_sum=1e6):
        ...
        tpmfn = str(self.paths["gene_spectra_tpm__txt"]) % (k, density_threshold_repl)
        ...
        save_df_to_npz(ref_spectra, str(self.paths["starcat_spectra"]) % (k, density_threshold_repl))
        save_df_to_text(
            ref_spectra,
            str(self.paths["starcat_spectra__txt"]) % (k, density_threshold_repl),
        )
    ...
    def load_results(self, K, density_threshold, n_top_genes=100, norm_usage=True):
        ...
        """
        scorefn = str(self.paths["gene_spectra_score__txt"]) % (
            K,
            str(density_threshold).replace(".", "_"),
        )
        tpmfn = str(self.paths["gene_spectra_tpm__txt"]) % (
            K,
            str(density_threshold).replace(".", "_"),
        )
        usagefn = str(self.paths["consensus_usages__txt"]) % (
            K,
            str(density_threshold).replace(".", "_"),
        )
        ...
        try:
            ...
            msg = "Usage matrix columns include non integer values"
